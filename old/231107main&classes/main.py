...
if __name__ == '__main__':
    ...
    image = cv2.imread(database_path + "/card.png")
    cropped_img = image[90:310, 420:600]
    cv2.imwrite(database_path + "face.png",cropped_img)
    #정보의 양을 늘리는 것(즉, 해상도를 늘리는 것)은 불가능하기에 필터링으로 이미지를 선명하게 변환
    improve1 = cv2.detailEnhance(cropped_img, sigma_s=10, sigma_r=0.15)
    cv2.imwrite(database_path + "improved_face.png", improve1)

    #sharpening(선명하게)한 것, 제일 쓸만한듯
    kernel = np.array([[0, -1, 0], [-1, 5, -1], [0, -1, 0]]) 
    improve3 = cv2.filter2D(cropped_img, -1, kernel) 
    cv2.imwrite(database_path + "sharpened_face.png", improve3)

    mustBeRemovedFile = database_path + "*.pkl"  # image 폴더 내의 *.pkl 확장자 파일을 삭제해주어야 함.

    for f in glob(mustBeRemovedFile):
        os.remove(f)

    fd = faceDetect.faceDetect(database_path)
    ...
    print(fd.match_face(database_path + "sharpened_face.png", "captured.png", name=hi.name))
    print(fd.match_face(database_path + "improved_face.png", "captured.png", name=hi.name))
    print(fd.match_face(database_path + "face.png", "captured.png", name=hi.name))
